...
titanic <- read.csv('/Users/Derek/Documents/Programming/HES/CSCI 106/Final Project/Titanic_Survival_Data.csv')
str(titanic)
...
ggplot(odata, aes(age)) +
  geom_histogram(bins=30)

# Bar chart showing that more females survived than males.
ggplot(odata, aes(survived)) +
  geom_bar()

# Bar chart showing that a higher number of people survived when they had less siblings on board.
ggplot(data.clean, aes(sibsp, survived)) +
  geom_bar(stat='identity')

ggplot(data.clean, aes(age, sibsp)) +
  geom_point()

ggplot(data.clean, aes(age, parch)) +
  geom_point()

ggplot(data.clean, aes(age, fare)) +
  geom_point()

ggplot(data.clean, aes(sibsp, parch)) +
  geom_point()

ggplot(data.clean, aes(sibsp, fare)) +
  geom_point()

ggplot(data.clean, aes(parch, fare)) +
  geom_point()

#install.packages("psych")
library(psych) # Reference 4 to understand how this works
tetrachoric(data.clean[, c("survived", "sex_M")])

#install.packages("polycor")
library(polycor) # Reference 4 to understand how this works
polychor(odata$survived, odata$pclass)

#install.packages("rcompanion")
library(rcompanion) # Reference 4 to understand how this works.
cramerV(odata$survived, odata$sex)
cramerV(odata$survived, odata$pclass) # most likely needs to be polychor
cramerV(odata$survived, odata$embarked)
cramerV(odata$survived, odata$deck)

cor_matrix <- cor(data.clean)#[,1]
...
```{r Data exploration on train subset}
# Histogram showing that more people in their late teens up to late thirties survived.
...
mulvar_model <- lm(survived ~ .-deck_G, data = train)
summary(mulvar_model)
vif(mulvar_model)

#lmod <- glm(as.factor(survived) ~ ., family = binomial, data = train)
lmod <- glm(as.factor(survived) ~ .-deck_G, family = binomial, data = train)
summary(lmod)
vif(lmod)
...
predictions_log_train <- ifelse(y_hat_mulvar_train > 0.5, 1, 0)
...
# Bar chart showing that more females survived than males.
ggplot(train, aes(sex_M, survived)) +
  geom_bar(stat='identity')
...
ggplot(train, aes(pclass, survived)) +
  geom_point()
...
(REF 4) "https://www.statology.org/correlation-between-categorical-variables/"
