...
# Sets the same seed for the random sampling after, useful for reproducability. 
set.seed(567)
...
```{r}
# develop model matrix using test data, this is only used for the predictions on
# the regularized models:
x.test = model.matrix(siri ~., data = q1.test)

# Get the predictions on non-regularized models:
# NOTE: the data fed is just the test df and you use the kwarg "newdata"
y_hat.mlr = predict(q1.model, newdata = q1.test)
y_hat.stepwise = predict(q1.model.stepwise, newdata = q1.test)
y_hat.robust = predict(q1.model.huber , newdata = q1.test)

# Get prediction on regularized models:
# NOTE: the data fed is a model matrix using the df and you use the kwarg "newx"
y_hat.ridge = predict(q1.model.ridge, s = best.lambda.ridge, newx = x.test)
y_hat.lasso = predict(q1.model.lasso, s = best.lambda.lasso, newx = x.test)
y_hat.enet = predict(q1.model.elastic , s = best.lambda.enet, newx = x.test)

# Model Performance on the test Data
library(caret)

## Linear Regression Model
ModelTest.reg = data.frame(obs = q1.test$siri, pred= y_hat.mlr)
reg = defaultSummary(ModelTest.reg)

## Stepwise Regression Model
ModelTest.stepwise = data.frame(obs = q1.test$siri, pred= y_hat.stepwise)
stepwise = defaultSummary(ModelTest.stepwise)

## Robust Regression Model
ModelTest.robust = data.frame(obs = q1.test$siri, pred= y_hat.robust)
robust = defaultSummary(ModelTest.robust)

## Ridge Regression Model
# NOTE: y_hat.ridge[,1] this converts the single col. matrix into a vector
# that can be read made into a col. for the data.frame
ModelTest.ridge = data.frame(obs = q1.test$siri, pred=y_hat.ridge[,1])
ridge = defaultSummary(ModelTest.ridge)

## Lasso Regression Model
ModelTest.lasso<-data.frame(obs = q1.test$siri, pred=y_hat.lasso[,1])
lasso<-defaultSummary(ModelTest.lasso)

## Elastic Net Regression Model
ModelTest.enet<-data.frame(obs = q1.test$siri, pred=y_hat.enet[,1])
enet<-defaultSummary(ModelTest.enet)
data.frame(rbind(reg, stepwise, robust, ridge, lasso, enet))
```
``` {r poisson}
  library(MASS)
  ...
  poissonReg <- glm(survived~.-deck_G,family=poisson, train)
  summary(poissonReg)
...
\par Since we have a p-value of 0, we reject the null hypothesis. Variables are important.

\par TODO - will update
\par According to the regression, .

\par Assuming a alpha of 0.05, when adding sequentially to the model, fare variable (a p-value of 2.267e-07), pclass_1 variable (a p-value of 2.018e-08), pclass_2 variable (a p-value of 3.622e-05), sex_M variable (a p-value of < 2.2e-16), deck_E variable (a p-value of 1.789e-13), deck_F variable (a p-value of 0.0002903)  are significant. 

\par Additionally, when performing the drop one test, we see that age variable (a p-value of 0.0006444), pclass_2 variable (a p-value of 0.0110067), sex_M variable (a p-value of < 2.2e-16), and deck_F variable (a p-value of 0.0011530) are significant. 

\par For the Poisson Regression, only sex_male is significant. TODO

\par Our Poisson Regression has a balanced accuracy of 81.40% with optimal cutoﬀ(.28) based on max F1 score (0.7636).
